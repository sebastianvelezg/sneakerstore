...
<div class="bg-dark text-light pt-5">
  <div class="container">
    <div class="row d-flex justify-content-center align-items-center" style="min-height: calc(40vh - 3rem)">
      <div class="col-md-6">
        <h1 class="text-center">{{ $viewData['accessory']->getType()}}</h1>
        ...
        <div class="col-md-6">
          <img
            src="{{ URL::to('/') }}/image/accessories/{{ $viewData['accessory']->getId() }}/{{ $viewData['accessory']->getImage() }}"
      ...
      @for ($i = 0; $i < count($viewData['images']); $i += 2)
        <div class="carousel-item {{ $i == 0 ? 'active' : '' }}">
          @if ($i + 1 < count($viewData['images']))
            <div class="d-flex">
              <img
                src="{{ URL::to('/') }}/image/accessories/{{ $viewData['accessory']->getId() }}/{{ $viewData['images'][$i]->getFilename() }}"
                style="height: 50vh; object-fit: cover" class="d-block w-100" alt="..." />
              <img
                src="{{ URL::to('/') }}/image/accessories/{{ $viewData['accessory']->getId() }}/{{ $viewData['images'][$i + 1]->getFilename() }}"
                style="height: 50vh; object-fit: cover" class="d-block w-100" alt="..." />
            </div>
          @else
            <img
