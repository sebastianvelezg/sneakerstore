...
@section('content')
    ...
    <div class="d-flex flex-wrap justify-content-around">
      @foreach ($viewData['categories'] as $category)
       @if($category->getType() == "accessory")
        <div class="card" style="width: 40%;">
          <div class="card-header">
            <h3 class="card-title">{{ $category->getName() }}</h3>
            <div class="card-tools">
              <a href="{{ route('admin.accessoryCreate', ['id'=> $category->getId()]) }}" class="btn btn-dark mb-3">
              Create
                <i class="fas fa-plus"></i>
              </a>
              <a href="{{ route('admin.accessoriesCategory', $category->getId()) }}" class="btn btn-dark mb-3">
              See All
